	...
	set_body() {
		...
		frappe.model.with_doctype(this.document_type, () => {
			...
			frappe.call(args).then((r) => {
				...
				// Create a dropdown instead of a list
				let $dropdown = $('<select class="quick-list-dropdown"></select>');
				data.forEach((doc) => {
					let $option = $('<option></option>').text(strip_html(doc[this.title_field_name])).val(doc.name);
					$dropdown.append($option);
				});
				let $textOption = $('<option value="" disabled selected="selected">Select a Project</option>');
    			$dropdown.append($textOption);

				$dropdown.change((e) => {
					let selectedValue = $dropdown.val();
					if (selectedValue) {
						frappe.set_route(`${frappe.utils.get_form_link(this.document_type, selectedValue)}`);
					}
				});
	
				this.body.append($dropdown);
			});
		});
	}
